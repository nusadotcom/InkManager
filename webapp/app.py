...
def upload_ficheiro(id_c):
    ...
    try:
        ...
        # Se a coluna id_marcacao não existir ainda, tenta sem ela
        try:
...
def editar_marcacao(id_m):
    ...
    if not marcacao:
        flash("Marcacao nao encontrada.", "erro")
        return redirect(url_for("marcacoes"))
    cur.execute("SELECT id, nome FROM clientes ORDER BY nome")
    clientes_lista = cur.fetchall()
    if request.method == "POST":
        ...
        materiais = request.form.get("materiais") or None
        duracao   = request.form.get("duracao") or None
        estado    = request.form.get("estado", marcacao["estado"])
        notas     = request.form.get("notas") or None
        if not id_c or not data_hora:
            flash("Cliente e data/hora sao obrigatorios.", "erro")
        else:
            try:
                if valor: valor = float(valor)
                cur2 = conn.cursor()
                cur2.execute("""UPDATE marcacoes SET id_cliente=%s, data_hora=%s, tipo=%s,
                    descricao=%s, valor=%s, materiais=%s, duracao=%s, estado=%s, notas=%s
                    WHERE id=%s""",
                    (id_c, data_hora, tipo, descricao, valor, materiais, duracao, estado, notas, id_m))
                conn.commit(); cur2.close()
                flash("Marcacao atualizada!", "sucesso")
                cur.close(); conn.close()
                return redirect(url_for("marcacoes"))
            except Exception as e:
                flash(f"Erro: {e}", "erro")
    if marcacao.get("data_hora"):
        marcacao["data_hora_fmt"] = marcacao["data_hora"].strftime("%Y-%m-%dT%H:%M")
    else:
        marcacao["data_hora_fmt"] = ""
    cur.close(); conn.close()
    return render_template("form_marcacao.html", clientes=clientes_lista, marcacao=marcacao, titulo="Editar Marcacao")
@app.route("/marcacoes/<int:id_m>/editar", methods=["GET","POST"])
